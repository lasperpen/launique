...
function loadnhac() {
    $('#qtloadingvds').removeClass('an');
    ab = $('#linkvds').val();
    $('#thongtinnguoigui').load(ab + ' div.postprofile dl dt img');
    $('#thuviennhac').load(ab + ' div.postbody > .content:eq(0)', function() {
...
function qtantdytnew() {
    ...
    $('#vds_flash').blur(function() {
        vdsB = $(this).val();
        if (vdsB.search('http://') == -1 || (vdsB.search('zing.vn') == -1 && vdsB.search('nhaccuatui') == -1 && vdsB.search('nhac.vui') == -1)) {
            $(this).css('background-color', '#FF0000');
            $('#vds_kt_2').val('');
        } else {
            $(this).css('background-color', '#74FF2E');
    ...
    $('#vds_ntdyt').blur(function() {
        ...
        if ((vdsE.length < 10) || (vdsE.search(' ') == -1)) {
            $(this).css('background-color', '#FF0000');
            $('#vds_kt_4').val('');
        } else {
            $(this).css('background-color', '#74FF2E');
...
$(document).ajaxComplete(function() {
    $("#vds_qtan a[href*='http://mp3.zing.vn/'], #vds_qtan a[href*='https://mp3.zing.vn/']").replaceWith(function() {
        return '<iframe width="100%" height="66" src="' + this.href + '" allowfullscreen="allowfullscreen" frameborder="0"></iframe>'
    })
});
